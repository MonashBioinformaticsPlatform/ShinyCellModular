# This tab mirrors "cellinfo_geneexpr" (RNA), same logic, ATAC assay
...
scDRnum_atac <- function(inpConf, inpMeta, inp1, inp2, inpsub1, inpsub2,
...
# Plot peak accessibility on dimred 
scDRpeak <- function(inpConf, inpMeta, inpdrX, inpdrY, inp1, inpsub1, inpsub2, 
                     inpH5, inpGene, 
...
# Plot cell information on dimred (ATAC copy, args are generic so logic is unchanged)
...
cellinfo_peakaccess_ui <- function(id, sc1conf, sc1def) {
  
  ns <- NS(id)
  
  tabPanel(
    title = HTML("CellInfo vs PeakAccess"),
    
    h4("Cell information vs peak accessibility on reduced dimensions"),
    ...
    fluidRow(
      ...
      column(
          ...
          column(
            ...
            selectInput(ns("sc1a1inp2"), "Peak name:", choices = NULL) %>%
              helper(
                ...
                content = c(
                  "Select peak to colour cells by peak accessibility",
                  paste0(
                    "Peak accessibility are coloured in a ",
        ...
        ######################## ??? MARKERPEAKS
        actionButton(ns("sc1a1tog10"), "Show Marker Peaks Per Cluster"),
...
cellinfo_peakaccess_server <- function(id, sc1conf_atac, sc1meta_atac, sc1gene_atac, sc1def_atac, dir_inputs, markerpeaks_lists) {
  moduleServer(id, function(input, output, session) {
    ...
    updateSelectizeInput(session, "sc1a1inp2", choices = names(sc1gene_atac), server = TRUE, 
    ...
    # Render plot 2 (peak accessibility) and provide the UI container
...
register_tab(
  ...
  description = "Cell metadata visualisation against peak accessibility, ATAC assay version of cellinfo_geneexpr",
  version     = "1.0",
  date        = "Jul 2026",
