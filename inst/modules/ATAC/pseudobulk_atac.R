# This tab mirrors "pseudobulk" (RNA), same logic, ATAC assay
# id     = "pseudobulk_atac"
# title  = "Pseudobulk DE (voom limma) (ATAC)"
...
sc_resolve_meta_col <- function(sc1conf_atac, sc1meta_atac, x) {
  if (is.null(x) || is.na(x) || !nzchar(x)) return(NULL)
  
  if (!is.null(sc1conf_atac) && "UI" %in% names(sc1conf_atac) && x %in% sc1conf_atac$UI) {
    id <- sc1conf_atac[UI == x]$ID
    if (length(id) == 1 && id %in% names(sc1meta_atac)) return(id)
  }
  
  if (x %in% names(sc1meta_atac)) return(x)
...
sc_make_pseudobulk <- function(sc1conf_atac,
  ...
  unit_col <- sc_resolve_meta_col(sc1conf_atac, sc1meta_atac, unit_col_ui_or_meta)
  cond_col <- sc_resolve_meta_col(sc1conf_atac, sc1meta_atac, condition_col_ui_or_meta)
  ...
  unit_vec <- as.character(sc1meta_atac[[unit_col]][keep_cells_idx])
  cond_vec <- as.character(sc1meta_atac[[cond_col]][keep_cells_idx])
  ...
  covar_cols <- vapply(covar_cols_ui_or_meta, function(z) sc_resolve_meta_col(sc1conf_atac, sc1meta_atac, z), character(1))
  ...
  if (length(covar_cols) > 0) {
    covar_df <- lapply(covar_cols, function(col) {
      v <- sc1meta_atac[[col]][keep_cells_idx]
  ...
  gene_idx <- as.integer(unname(sc1gene_atac))
  gene_names <- names(sc1gene_atac)
  shiny::validate(shiny::need(length(gene_idx) > 1, "sc1gene_atac is empty or invalid."))
...
pseudobulk_atac_ui <- function(id, sc1conf_atac, sc1def_atac) {
  
  ns <- NS(id)
  
  tabPanel(

    HTML("Pseudobulk DE (voom limma) (ATAC)"),
    ...
    tags$div(
      ...
      tags$p(style = "margin-top: 0;",
             ...
             " Tests which peaks are differentially accessible between two conditions",
      ...
      tags$p(
        ...
        " Then we use standard statistical methods (voom + limma) to find peaks that are",
      ...
      tags$ol(style = "margin-top: 4px; padding-left: 20px;",
              tags$li(tags$strong("Pick a cell type"), " — you're asking: in this cell type, what peaks change between conditions?"),
    ...
    fluidRow(
      column(
        ...
        selectInput(
          ...
          selected = sc1conf_atac[grp == TRUE]$UI[1]
        ),
        ...
        selectInput(
          ...
          choices = unique(c(sc1conf_atac$UI)),
          selected = if ("orig.ident" %in% unique(c(sc1conf_atac$UI))) "orig.ident" else unique(c(sc1conf_atac$UI))[1]
        ),
        ...
        conditionalPanel(
          ...
          checkboxInput(
            ...
            tagList(
              "Robust eBayes",
              info_icon("Uses robust empirical Bayes estimation, more resistant to outlier peaks. Recommended for most analyses. Disable only with very few peaks.")
            ),
            value = TRUE
          )
        ),
        ...
        conditionalPanel(
          ...
          numericInput(
            ...
            tagList(
              "FDR cutoff",
              info_icon("Adjusted p-value threshold. Peaks below this FDR are coloured in volcano and MA plots. Default 0.05.")
            ),
...
pseudobulk_atac_server <- function(id, sc1conf_atac, sc1meta_atac, sc1gene_atac, sc1def_atac, dir_inputs) {
  moduleServer(id, function(input, output, session) {
    ...
    output$sc1e1_celltype_level.ui <- renderUI({
      ...
      col <- sc_resolve_meta_col(sc1conf_atac, sc1meta_atac, input$sc1e1_celltype_col)
      req(col)
      
      lv <- as.character(sc1meta_atac[[col]])
    ...
    available_cond_levels <- reactive({
      ...
      cond_col <- sc_resolve_meta_col(sc1conf_atac, sc1meta_atac, input$sc1e1_condition_col)
      req(cond_col)
      
      cond_vec <- as.character(sc1meta_atac[[cond_col]][keep])
        ...
        tagList(
          ...
          selectInput(
            ...
            tagList(
              ...
              info_icon("Baseline group. Positive logFC means a peak is more accessible in Group 2 relative to this group.")
            ),
            choices = cond_levels,
          ...
          selectInput(
            ...
            tagList(
              ...
              info_icon("Group compared against the reference. Peaks with positive logFC are more accessible in this group.")
            ),
            choices = cond_levels,
    ...
    preview_design <- reactive({
      ...
      unit_col <- sc_resolve_meta_col(sc1conf_atac, sc1meta_atac, input$sc1e1_unit_col)
      cond_col <- sc_resolve_meta_col(sc1conf_atac, sc1meta_atac, input$sc1e1_condition_col)
      req(unit_col, cond_col)
      
      unit_vec <- as.character(sc1meta_atac[[unit_col]][keep])
      cond_vec <- as.character(sc1meta_atac[[cond_col]][keep])
      ...
      if (length(covars) > 0) {
        ok2 <- !is.na(as.character(sc1meta_atac[[unit_col]][keep])) & nzchar(as.character(sc1meta_atac[[unit_col]][keep])) &
               !is.na(as.character(sc1meta_atac[[cond_col]][keep])) & nzchar(as.character(sc1meta_atac[[cond_col]][keep]))
        cond_full <- as.character(sc1meta_atac[[cond_col]][keep])[ok2]
        unit_full <- as.character(sc1meta_atac[[unit_col]][keep])[ok2]
        keep_ok <- keep[ok2]
        ...
        covar_cols <- vapply(covars, function(z) sc_resolve_meta_col(sc1conf_atac, sc1meta_atac, z), character(1))
        covar_cols <- covar_cols[!is.na(covar_cols) & nzchar(covar_cols)]
        covar_cols <- unique(covar_cols)
        
        if (length(covar_cols) > 0) {
          covar_df <- lapply(covar_cols, function(col) {
            v <- as.character(sc1meta_atac[[col]][keep_ok])
...
register_tab(
  ...
  description = "Pseudobulk differential accessibility analysis, ATAC assay version of pseudobulk",
  version     = "1.0",
  date        = "Jul 2026",
  source      = "MGBP custom, ATAC adaptation",
