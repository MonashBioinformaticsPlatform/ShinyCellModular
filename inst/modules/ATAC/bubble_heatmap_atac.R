# This tab mirrors "bubble_heatmap" (RNA), same logic, ATAC assay
# id     = "bubble_heatmap_atac",
# title  = "Bubbleplot / Heatmap (ATAC)",

############################################### Functions ###########################################

# Get peak list
scPeakList <- function(inp, inpGene) {
...
# Plot peak accessibility bubbleplot / heatmap
scBubbHeatPeak <- function(inpConf, inpMeta, inp, inpGrp, inpPlt,
  ...
  geneList <- scPeakList(inp, inpGene)
...
bubble_heatmap_atac_ui <- function(id, sc1conf_atac, sc1def_atac) {
  
  ns <- shiny::NS(id)
  
  tabPanel(
    HTML("Bubbleplot / Heatmap (ATAC)"),
    h4("Peak accessibility bubbleplot / heatmap"),
    "In this tab, users can visualise the peak accessibility patterns of ",
    "multiple peaks grouped by categorical cell information (e.g. library / cluster).", br(),
    ...
    fluidRow(
      column(
        ...
        textAreaInput(
          ns("sc1d1inp"),
          HTML("List of peak names <br />
               ( separated <br />
               by , or ; or newline):"),
          height = "200px",
          value = paste0(sc1def_atac$genes, collapse = ", ")
        ),
        ...
        checkboxInput(ns("sc1d1scl"), "Scale peak accessibility", value = TRUE),
        checkboxInput(ns("sc1d1row"), "Cluster rows (peaks)", value = TRUE),
...
bubble_heatmap_atac_server <- function(id, sc1conf_atac, sc1meta_atac, sc1gene_atac, sc1def_atac, dir_inputs) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    x_levels_default <- reactive({
      req(input$sc1d1grp)
      x_id <- sc1conf_atac[UI == input$sc1d1grp]$ID
    ...
    output$sc1d1sub1.ui <- renderUI({
      req(input$sc1d1sub1)
      sub <- strsplit(sc1conf_atac[UI == input$sc1d1sub1]$fID, "\\|")[[1]]
    ...
    output$sc1d1oupTxt <- renderUI({
      geneList <- scPeakList(input$sc1d1inp, sc1gene_atac)
      ...
      oup <- paste0(nrow(ok), " peaks OK and will be plotted")
      if (nrow(notok) > 0) {
        oup <- paste0(
          oup, "<br/>",
          nrow(notok), " peaks not found (",
...
register_tab(
  id          = "bubble_heatmap_atac",
  title       = "Bubble Plot / Heatmap (ATAC)",
  ui          = bubble_heatmap_atac_ui,
  server      = bubble_heatmap_atac_server,
  author      = "Laura Perlaza-Jimenez",
  description = "Bubble plot and heatmap visualisation of peak accessibility, ATAC assay version of bubble_heatmap. Allows ordering and Flexplots",
  version     = "1.0",
  date        = "Jul 2026",
