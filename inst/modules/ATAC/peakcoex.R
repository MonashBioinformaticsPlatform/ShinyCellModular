# This tab mirrors "genecoex" (RNA), same logic, ATAC assay
# id     = "peakcoex",
# title  = "Peak Coaccessibility",

##################################################### Functions #########################################################
# Plot peak coaccessibility on dimred 
...
scDRpeakcoex <- function(inpConf, inpMeta, inpdrX, inpdrY, inp1, inp2, inpsub1, inpsub2, inpH5, inpGene, inpsiz, inpcol, inpord, inpfsz, inpasp, inptxt){ 
  ...
  if(cInp[1] == "Red (Peak1)"){ 
    c10 = c(255,0,0) 
  } else if(cInp[1] == "Orange (Peak1)"){ 
    c10 = c(255,140,0) 
  } else { 
    c10 = c(0,255,0) 
  } 
  if(cInp[2] == "Green (Peak2)"){ 
    c01 = c(0,255,0) 
  } else { 
    c01 = c(0,0,255) 
  } 
...
scDRpeakcoexLeg <- function(inp1, inp2, inpcol, inpfsz){ 
  # Generate coex color palette 
  cInp = strsplit(inpcol, "; ")[[1]] 
  if(cInp[1] == "Red (Peak1)"){ 
    c10 = c(255,0,0) 
...
scDRpeakcoexNum <- function(inpConf, inpMeta, inp1, inp2, inpsub1, inpsub2, inpH5, inpGene){ 
  if(is.null(inpsub1)){inpsub1 = inpConf$UI[1]} 
  # Prepare ggData 
...
peakcoex_ui <- function(id, sc1conf_atac, sc1def_atac){
  ns <- NS(id)
  
  tabPanel( 
    HTML("Peak coaccessibility"), 
    h4("Coaccessibility of two peaks on reduced dimensions"), 
    "In this tab, users can visualise the coaccessibility of two peaks ", 
    ...
    fluidRow( 
      column( 
        ...
        fluidRow( 
          column( 
            12, selectInput(ns("sc1b2drX"), "X-axis:", choices = sc1conf_atac[dimred == TRUE]$UI, 
                            selected = sc1def_atac$dimred[1]), 
            selectInput(ns("sc1b2drY"), "Y-axis:", choices = sc1conf_atac[dimred == TRUE]$UI, 
    ...
    fluidRow( 
      column( 
        3, style="border-right: 2px solid black", h4("Peak Accessibility"), 
        selectInput(ns("sc1b2inp1"), "Peak 1:", choices=NULL) %>%  
          helper(type = "inline", size = "m", fade = TRUE, 
                title = "Peak accessibility to colour cells by", 
                content = c("Select peak to colour cells by peak accessibility", 
                            paste0("- Peak accessibility are coloured in a ", 
                                  "White-Red colour scheme which can be ", 
                                  "changed in the plot controls"))), 
        selectInput(ns("sc1b2inp2"), "Peak 2:", choices=NULL) %>% 
          helper(type = "inline", size = "m", fade = TRUE, 
                  title = "Peak accessibility to colour cells by", 
                  content = c("Select peak to colour cells by peak accessibility", 
                              paste0("- Peak accessibility are coloured in a ", 
        ...
        conditionalPanel( 
          ...
          radioButtons(ns("sc1b2col1"), "Colour:", 
                        choices = c("Red (Peak1); Blue (Peak2)", 
                                    "Orange (Peak1); Blue (Peak2)", 
                                    "Red (Peak1); Green (Peak2)", 
                                    "Green (Peak1); Blue (Peak2)"), 
                        selected = "Red (Peak1); Blue (Peak2)"), 
...
peakcoex_server <-  function(id, sc1conf_atac, sc1meta_atac, sc1gene_atac, sc1def_atac, dir_inputs) {
    moduleServer(id, function(input, output, session) {
      ...
      updateSelectizeInput(session, "sc1b2inp1", choices = names(sc1gene_atac), server = TRUE, 
                           selected = sc1def_atac$gene1, options = list( 
                             maxOptions = 7, create = TRUE, persist = TRUE, render = I(optCrt))) 
      updateSelectizeInput(session, "sc1b2inp2", choices = names(sc1gene_atac), server = TRUE, 
                           selected = sc1def_atac$gene2, options = list( 
                             maxOptions = 7, create = TRUE, persist = TRUE, render = I(optCrt))) 
  ...
  output$sc1b2sub1.ui <- renderUI({ 
    sub = strsplit(sc1conf_atac[UI == input$sc1b2sub1]$fID, "\\|")[[1]] 
  ...
  output$sc1b2oup1 <- renderPlot({ 
    scDRpeakcoex(sc1conf_atac, sc1meta_atac, input$sc1b2drX, input$sc1b2drY,   
             input$sc1b2inp1, input$sc1b2inp2, input$sc1b2sub1, input$sc1b2sub2, 
             file.path(dir_inputs,"ATAC","sc1gexpr.h5"), sc1gene_atac, 
  ...
  output$sc1b2oup1.pdf <- downloadHandler( 
    ...
    content = function(file) { ggsave( 
      ...
      plot = scDRpeakcoex(sc1conf_atac, sc1meta_atac, input$sc1b2drX, input$sc1b2drY,  
                      input$sc1b2inp1, input$sc1b2inp2, input$sc1b2sub1, input$sc1b2sub2, 
                      file.path(dir_inputs,"ATAC","sc1gexpr.h5"), sc1gene_atac, 
                      input$sc1b2siz, input$sc1b2col1, input$sc1b2ord1, 
                      input$sc1b2fsz, input$sc1b2asp, input$sc1b2txt) ) 
  ...
  output$sc1b2oup2 <- renderPlot({ 
    scDRpeakcoexLeg(input$sc1b2inp1, input$sc1b2inp2, input$sc1b2col1, input$sc1b2fsz) 
  }) 
  ...
  output$sc1b2oup2.pdf <- downloadHandler( 
    ...
    content = function(file) { ggsave( 
      ...
      plot = scDRpeakcoexLeg(input$sc1b2inp1, input$sc1b2inp2, input$sc1b2col1, input$sc1b2fsz) ) 
  }) 
  ...
  output$sc1b2.dt <- renderDataTable({ 
    ggData = scDRpeakcoexNum(sc1conf_atac, sc1meta_atac, input$sc1b2inp1, input$sc1b2inp2, 
                         input$sc1b2sub1, input$sc1b2sub2, file.path(dir_inputs,"ATAC","sc1gexpr.h5"), sc1gene_atac) 
...
register_tab(
  ...
  description = "Peak coaccessibility visualisation on reduced dimensions, ATAC assay version of genecoex",
  version     = "1.0",
  date        = "Jul 2026",
