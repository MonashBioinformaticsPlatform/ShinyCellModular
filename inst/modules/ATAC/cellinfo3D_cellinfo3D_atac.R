# CellInfo3D vs CellInfo3D (ATAC)
# This tab mirrors "cellinfo3D_cellinfo3D" (RNA), same logic, ATAC assay
# id     = "cellinfo3D_cellinfo3D_atac",
# title  = "CellInfo3D vs CellInfo3D (ATAC)",
...
scDRcell3D_atac <- function(inpConf, inpMeta, dr3d_base,
...
cellinfo3D_cellinfo3D_atac_ui <- function(id, sc1conf_atac, sc1def_atac) {
  ns <- NS(id)
  
  tabPanel(
    HTML("CellInfo3D vs CellInfo3D (ATAC)"),
        ...
        fluidRow(
          column(6,
            selectInput(ns("sc1a2inp1"), "Cell information:",
                        choices = sc1conf_atac$UI, selected = sc1def_atac$meta1)
          ),
          column(6,
        ...
        fluidRow(
          column(6,
            selectInput(ns("sc1a2inp2"), "Cell information:",
                        choices = sc1conf_atac$UI, selected = sc1def_atac$meta2)
          ),
          column(6,
...
cellinfo3D_cellinfo3D_atac_server <- function(id, sc1conf_atac, sc1meta_atac, sc1gene_atac, sc1def_atac, dir_inputs) {
  moduleServer(id, function(input, output, session) {
    ...
    dr3d_tbl <- reactive({ scList3DReductions_atac(sc1conf_atac, sc1meta_atac) })
    ...
    # ── Plot reactives ──
    plot1 <- reactive({
      scDRcell3D_atac(sc1conf_atac, sc1meta_atac,
...
register_tab(
  id          = "cellinfo3D_cellinfo3D_atac",
  title       = "CellInfo3D vs CellInfo3D (ATAC)",
  ui          = cellinfo3D_cellinfo3D_atac_ui,
  server      = cellinfo3D_cellinfo3D_atac_server,
  author      = "Laura Perlaza-Jimenez",
  description = "3D cell metadata visualisation against cell metadata, ATAC assay version of cellinfo3D_cellinfo3D",
  version     = "1.0",
  date        = "Jul 2026",
