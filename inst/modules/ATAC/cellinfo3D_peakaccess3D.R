# CellInfo3D vs PeakAccess3D
# This tab mirrors "cellinfo3D_geneexpr3D" (RNA), same logic, ATAC assay
...
scList3DReductions_atac <- function(inpConf, inpMeta) {
...
scGet3DReduction_atac <- function(inpConf, inpMeta, base = NULL) {
  
  dt <- scList3DReductions_atac(inpConf, inpMeta)
...
scDRcell3D_atac <- function(inpConf, inpMeta,
                       dr3d_base,
  ...
  red3 <- scGet3DReduction_atac(inpConf, inpMeta, base = dr3d_base)
...
# Plot peak accessibility on dimred 3D
scDRpeak3D <- function(inpConf, inpMeta,
                       dr3d_base,
                       inp1, inpsub1, inpsub2,
                       inpH5, inpGene,
...
cellinfo3D_peakaccess3D_ui <- function(id, sc1conf_atac, sc1def_atac) {
  
  ns <- NS(id)
  
  tabPanel(
    title = HTML("CellInfo3D vs PeakAccess3D"),
    
    h4("Cell information vs peak accessibility on reduced dimensions 3D"),
    "In this tab, users can visualise both cell information and peak ",
    "accessibility side-by-side on low-dimensional representions in 3D.",
    br(), br(),
    ...
    fluidRow(
      ...
      column(
        ...
        radioButtons(
          ...
          choices  = c("Cell Info" = "plot1", "Peak Access" = "plot2"),
    ...
    fluidRow(
      ...
      column(
        6, h4("Peak accessibility 3D"),
        fluidRow(
          column(
            6,
            selectInput(ns("sc1a1inp2"), "Peak name:", choices = NULL)
          ),
          column(
            6,
...
cellinfo3D_peakaccess3D_server <- function(id, sc1conf_atac, sc1meta_atac, sc1gene_atac, sc1def_atac, dir_inputs) {
  moduleServer(id, function(input, output, session) {
    ...
    updateSelectizeInput(
      session, "sc1a1inp2",
      choices = names(sc1gene_atac),
      server = TRUE,
      selected = sc1def_atac$gene1,
    ...
    dr3d_tbl <- reactive({
      scList3DReductions_atac(sc1conf_atac, sc1meta_atac)
    })
    ...
    output$sc1a1sub1_ui <- renderUI({
      req(input$sc1a1sub1)
      sub <- strsplit(sc1conf_atac[UI == input$sc1a1sub1]$fID, "\\|")[[1]]
    ...
    plot2 <- reactive({
      ...
      scDRpeak3D(
        ...
        inpsub1 = input$sc1a1sub1,
        inpsub2 = input$sc1a1sub2,
        inpH5   = file.path(dir_inputs,"ATAC","sc1gexpr.h5"),
        inpGene = sc1gene_atac,
        inpsiz  = input$sc1a1siz,
...
register_tab(
  ...
  description = "3D cell metadata visualisation against peak accessibility, ATAC assay version of cellinfo3D_geneexpr3D",
  version     = "1.0",
  date        = "Jul 2026",
